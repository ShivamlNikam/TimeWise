...
--
-- Temporary view structure for view `idclectureslot`
--

DROP TABLE IF EXISTS `idclectureslot`;
/*!50001 DROP VIEW IF EXISTS `idclectureslot`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8mb4;
/*!50001 CREATE VIEW `idclectureslot` AS SELECT 
 1 AS `cid`,
 1 AS `fid`,
 1 AS `dept`,
 1 AS `year_of_study`,
...
--
-- Final view structure for view `idclectureslot`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `idclectureslot` AS select `class`.`cid` AS `cid`,`slot`.`fid` AS `fid`,`class`.`dept` AS `dept`,`class`.`year_of_study` AS `year_of_study`,`class`.`division` AS `division`,`slot`.`room_no` AS `room_no`,`subject`.`subjid` AS `subjid`,`subject`.`subj_name` AS `subj_name`,`slot`.`weekday` AS `weekday`,`slot`.`start_time` AS `start_time`,`slot`.`end_time` AS `end_time` from ((`class` join `slot` on(((`slot`.`cid` = `class`.`cid`) and (`slot`.`flag` = 'idc')))) join `subject` on((`subject`.`subjid` = `slot`.`subjid`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2019-10-01  1:24:34
