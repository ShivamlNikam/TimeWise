...
function putElectiveAssignedMap(){
    ...
    <div class="electiveTable" id="elTable">
        <table border="3" id="elective_table">
          <th>${subjName.split(':')[1]}</th>
          <tr>
            <td align="center" id="ele-theory"></td>
          </tr>
          <tr>
            <td align="center" id="ele-practical"></td>
          </tr>
        </table>
    </div>
    `;
    ...
    if(slot=='ELE-THEORY' || slot=='ELE-PRACTICAL'){
        ...
        xhr.onreadystatechange = function(){
            if(xhr.readyState==4 && xhr.status==200){
                ...
                document.getElementById('ele-theory').innerHTML=`THEORY: ${electiveMap.get('THEORY')}/${conditionMap.get('THEORY')}`;
                document.getElementById('ele-practical').innerHTML=`PRACTICAL: ${electiveMap.get('PRACTICAL')}/${conditionMap.get('PRACTICAL')}`;
            }
        }
        xhr.send();
