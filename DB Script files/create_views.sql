...
create or replace view idcLectureSlot as
select class.cid, slot.fid, dept, year_of_study, division, room_no, subject.subjid, subj_name, weekday, start_time, end_time
from class inner join slot on slot.cid = class.cid and slot.flag = 'idc'
inner join subject on subject.subjid = slot.subjid;

select * from idcLectureSlot;
...
select * from subj_dept_sem
where subjid='CSM';
update subj_dept_sem
set lab =3
where subjid = 'CSM';
